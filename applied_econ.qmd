...
$$
\pi(p) = \sum^n_{i=1}\alpha_i p_i + \frac{1}{2}\left(\sum^n_{i=1}\theta_i p_i\right)^{-1}\sum^n_{i=1}\sum^n_{j=1}\beta_{ij}p_i p_j
$$
...
Cette forme fonctionnelle permet d'imposer globalement des conditions sur la courbure requise (si nécessaire permet d’imposer la convexité des prix)
...
L'utilisation de l'économétrie pour étudier la production, les coûts et le profit est très intéressante pour analyser les facteurs qui influent sur ces variables. On a notamment constaté que, bien que simple, la fonction de Cobb-Douglas fournit généralement beaucoup d'informations, contrairement à la fonction translog qui est plus complexe à interpréter et à utiliser en raison du grand nombre de coefficients impliqués, pour des résultats pas toujours très pertinents. De plus, dans les cas où l'on rencontre de l'inefficacité, on peut obtenir de meilleurs résultats avec un modèle SFA, que nous avons notamment utilisée pour estimer la production. Dans les faits, quelles que soient les variables étudiées, on ne peut pas affirmer qu'une fonction est meilleure qu'une autre. Certaines permettent d'estimer les **coûts marginaux** (Translog), d'autres la **productivité marginale** (Cobb-Douglas, Quadratique, Translog), les **élasticités de substitutions** (CES). C'est pourquoi nous pensons que dans le cas où l'on souhaite étudier les coûts et la production, il est nécessaire d'examiner tous ces outils et de croiser les résultats pour avoir une idée générale du sujet que l'on traite.
