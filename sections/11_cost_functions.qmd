...
-  Le coefficient de 0.279 pour `qOut` signifie que si la production augmente de 1%, le coût de augmentera de 0.279%, toutes choses étant égales par ailleurs. 
...
```{r}
#| echo: false
coefficients <- c("Intercept", "qOut", "pCap", "pLab", "pMat","qCap")
valeur_lt <- c(6.754, 0.373, 0.074, 0.465, 0.486, NA)
valeur_ct <- c(5.603, 0.279, NA, 0.402, 0.413, 0.237)

df_comparasion <- data.frame(coefficients = coefficients,
...
df_comparasion |> 
  gt() |> 
  tab_header(title = md("**Comparasion coefficients Cobb-Douglas**"),
             subtitle = md("Variable dépendante : `cost`")) |> 
  cols_label(coefficients = md("**Coefficient**"),
             long_terme = md("**LT**"),
             court_terme = md("**CT**")) |>
    tab_options(
