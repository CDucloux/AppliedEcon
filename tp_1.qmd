...
- [x] Changer l'écriture scientifique des labels des graphs vers une écriutre en millions (25M)
- [x] Continuer les interprétations de la partie d'Analyse Exploratoire
- [x] Pour la SFA, faire le blabla des inneficcacités $E(\epsilon|u)$
- [ ] Pour les fonctions de cout mettre les formes fonctionnelles avec nos 3 inputs à nous
- [x] Mettre les noms Corentin et Guillaume dans tous les plots 😎
...
L'analyse en composantes principales (ACP) que nous nous apprêtons à faire est justifiée dans notre contexte car nous n'avons que des variables numériques. Les deux premiers axes concentrent 60% de la variance. Nous allons alors limiter notre ACP à l'étude de ces deux axes.
...
- **Axe 2** $\Rightarrow$ Productivités moyennes (`AP_Cap`,`AP_Mat`,`AP_Lab`), prix de vente de la production (`pOut`)
...
Dans notre ACP, on ne constate pas de variables qui sont fortement opposées. Elle permet de mettre en avant le lien entre les quantités des facteurs de production et leur valeur. On observe également que le prix de vente est étroitement lié avec les productivités moyennes.
...
- On trouve que $\hat{\alpha} + \hat{\beta} + \hat{\gamma} =$ **`r round(return_to_scale, 2)`**, donc les rendements d'échelles sont croissants, c'est à dire que le processus de production est considéré comme particulièrement efficace et présente des *économies d’échelle*. Un accroissement identique de tous les facteurs conduit à un
...
#### Elasticité de substitution des inputs
...
​+\frac{1}{2}​(β_{11}\ln(​q^2_{Cap})​+β_{22}\ln(​q^2_{Lab})​+β_{33}\ln(​q^2_{Mat}))\\
...
Le test de rapport de vraisemblance est un test d'hypothèse qui compare l'adéquation de l'ajustement de deux modèles afin de déterminer celui qui offre le meilleur ajustement. Dans notre cas, on veut comparer le modèle **Cobb-Douglas** estimé par *Moindres Carrés Ordinaires* et le modèle **Cobb-Douglas SFA** estimé par la méthode du *Maximum de Vraisemblance*. 
...
-  Le coefficient de 0.373 pour `qOut` signifie que si la production augmente de 1%, le coût de augmentera de 0.373%, *ceteris paribus*.

- Le coefficient de `pLab` est de 0.464. Cela indique qu'une augmentation d'1% du prix du travail cela entraine une augmentation d'environ 0.464% du coût total, *ceteris paribus*.

- Le coefficient de `pMat` est de 0.486. Il indique qu'une augmentation d'1% du prix des matériaux cela entraine une augmentation d'environ 0.486% du coût total, *ceteris paribus*.
...
-  Le coefficient de 0.279 pour `qOut` signifie que si la production augmente de 1%, le coût de augmentera de 0.279%, toutes choses étant égales par ailleurs. On remarque donc que à court terme la variation de la quantité produite à moins d'impact sur le coût que à long terme.
...
### Fonction de coût Translog *(PARTIE BANCALE)*
...
[\frac{1}{4}\beta_{11}(\ln(q_{Out}))^2] \\ [(\beta_{22}\ln(p^2_{Cap})+\beta_{33}\ln(p^2_{Lab})+\beta_{44}\ln(p^2_{Mat})
+\beta_{23}\ln(p_{Cap}p_{Lab})+\beta_{24}\ln(p_{Cap}p_{Mat})+\beta_{34}\ln(p_{Lab}p_{Mat})] + \\
\frac{1}{2}[\beta_{12}\ln(p_{Cap})\ln(q_{Out})+ \beta_{13}\ln(p_{Lab})\ln(q_{Out}) + \beta_{14}\ln(p_{Mat})\ln(q_{Out})]+\epsilon_i
\end{gathered}
$$
...
- On remarque qu'avec une fonction de coût Translog, le coût dépend majoritairement de la quantité produite, on le constate avec la significativité des coefficients associés à `qOut` et `qOut^2`.
