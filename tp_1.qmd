...
- [ ] Test de Fisher à mettre en place dans la partie fonction de production Translog (vs Cobb Douglas)
- [ ] Rendements d'échelle pour la quadratique avec $\lambda$ très très bancal
- [ ] étude des VIF pour la translog form et du posterior predictive check
- [ ] Inclure aussi l'elasticité de substitution pour les fonctions de cost
...
Cependant, utiliser cette forme fonctionnelle peut s'avérer problématique car elle impose que toute paire d'inputs aient la même élasticité de substitution, soit par exemple que $\sigma_{\{\text{qCap, qLab}\}} = \sigma_{\{\text{qCap, qMat}\}}$, alors même 

- Tandis que la **fonction de production CES emboitée** proposée par @sato1967 dans notre cas s'écrit sous la forme :
...
La fonction de court terme est elle définie par l'immutabilité d'au moins un facteur de production. Dans notre contexte, le capital est fixe, ce qui nous permet de la caractériser ainsi.
...
On constate que le coût moyen diminue considérablement au début, puis se stabilise à mesure que la quantité produite augmente.
...
$\Rightarrow$ Estimer somehow la leontieff généralisée, vérifier hétéroscédasticité.
